...
def process_data_from_folder(data_folder_path):
    ...
    if 'schedule ship date' in orders_forecast_df.columns:
        try:
            orders_forecast_df['Filler'] = pd.to_datetime(orders_forecast_df['schedule ship date'], format= '%m/%d/%y', errors='coerce').dt.strftime('%b-%y')
    ...
    # Assign 'Intercompany' segment for internal customer types.
...
if __name__ == "__main__":
    ...
    data_folder = input('Input the folder path:').replace('\\','/')
